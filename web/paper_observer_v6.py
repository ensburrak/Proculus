#!/usr/bin/env python3
"""Proculus V6: localhost-only, paper EVIDENCE observer. No trading/control writes.

Copy Proculus_Immersive_v6.html into web/ and run:
...
def serve(repo: Path, site: Path, port: int):
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "ProculusPaperReadOnly/0.2"
        ...
        def do_GET(self):
            ...
            path = unquote(u.path)
            if path in {"/", "/Proculus_Immersive_v6.html"}:
                website = site / "Proculus_Immersive_v6.html"
                if not website.is_file():
                    return self.reply({"error": "copy_v6_website_to_web_folder"}, 404)
